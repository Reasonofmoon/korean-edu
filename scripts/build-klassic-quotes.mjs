...
function sanitizeQuote(item, index) {
  const extraHints = Array.isArray(item.extraHints)
    ? item.extraHints.map((hint) => String(hint).trim()).filter(Boolean)
    : [];
  return {
    ...
    source: item.source === "seed" || item.source === "seed-culture" ? "seed" : "klassic-quote-api",
    extraHints,
  };
}
...
async function main() {
  ...
  const enriched = combined
    .map((item) => {
      ...
      const autoHints = keywordHints(item.quote, item.name);
      const mergedHints = [...new Set([...(item.extraHints ?? []), ...autoHints])].slice(0, 14);
      const { extraHints: _drop, ...rest } = item;
      return {
        ...rest,
        contentRating,
        contentRatingLabel: ratingLabel(contentRating),
        keywordHints: mergedHints,
      };
    })
